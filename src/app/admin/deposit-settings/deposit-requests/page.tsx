...
export default function DepositRequestsPage() {
  ...
  const handleApprove = async (id: number) => {
    try {
      const response = await adminFetch(`/api/admin/wallet/deposit-requests/${id}/approve`, { 
        method: 'POST' 
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      const response = await adminFetch(`/api/admin/wallet/deposit-requests/${id}/reject`, { 
        method: 'POST' 
      });
      const data = await response.json();
      if (data.success) {
  ...
  const getStatusBadge = (status: string) => {
    const map = {
      ...
      approved: { label: '已充值', className: 'bg-green-600' },
      rejected: { label: '已拒绝', className: 'bg-red-600' },
    };
  ...
  const filteredRequests = requests.filter(req => {
    ...
    return (
      ...
      req.paymentAddress.toLowerCase().includes(searchLower) ||
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button 
            onClick={() => router.push('/admin/deposit-settings/deposit-requests/create')}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-left p-4 text-gray-400 font-medium cursor-pointer hover:text-white">
                    ID
                  </th>
              ...
              <tbody>
                {filteredRequests.map((req) => (
                  <tr key={req.id} className="border-b border-slate-700 hover:bg-slate-700/50">
                    ...
                    <td className="p-4 text-white">{req.email}</td>
