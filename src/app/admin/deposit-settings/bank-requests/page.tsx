...
export default function BankRequestsPage() {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const response = await adminFetch('/api/admin/wallet/bank-deposit-requests');
  ...
  const handleApprove = async (id: number) => {
    try {
      const response = await adminFetch(`/api/admin/wallet/bank-deposit-requests/${id}/approve`, { method: 'POST' });
      const data = await response.json();
      if (data.success) {
        toast.success('申请已通过');
        fetchRequests();
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      const response = await adminFetch(`/api/admin/wallet/bank-deposit-requests/${id}/reject`, { method: 'POST' });
      const data = await response.json();
      if (data.success) {
        toast.success('申请已拒绝');
        fetchRequests();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-gray-400">
        <span>桌面</span> / <span>充币设置</span> / <span className="text-white">银行卡充币申请</span>
      </div>
      <div>
        <h1 className="text-2xl font-bold text-white">银行卡充币申请</h1>
        <p className="text-gray-400 mt-1">查看和处理用户的银行卡充币申请</p>
      </div>
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-white">银行卡充币申请列表</CardTitle>
        ...
        <CardContent>
          {requests.map((req) => (
            <div key={req.id} className="p-4 bg-slate-700/50 rounded-lg mb-2">
              ...
              {req.proofImage && (
                <div className="mt-2">
                  <span className="text-gray-400 text-sm">转账凭证: </span>
