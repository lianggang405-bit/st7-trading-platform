...
import { Search, RefreshCw, ArrowUp, ArrowDown, TrendingUp, CheckCircle, XCircle, Clock, Play } from 'lucide-react';
import { toast } from 'sonner';
import adminFetch from '@/lib/admin-fetch';

interface DemoQuickContractTrade {
  id: number;
  userId: number;
  email: string;
  symbol: string;
...
export default function DemoQuickContractTradesPage() {
  const [trades, setTrades] = useState<DemoQuickContractTrade[]>([]);
  const [filteredTrades, setFilteredTrades] = useState<DemoQuickContractTrade[]>([]);
  ...
  const fetchTrades = async () => {
    ...
    try {
      ...
      const response = await adminFetch(`/api/admin/quick-contract/demo-trades?${params}`);
      ...
      console.error('Failed to fetch demo trades:', error);
      toast.error('获取模拟秒合约交易失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-gray-600">/</span>
        <span className="text-white">模拟秒合约交易</span>
      </div>

      <div>
        <h1 className="text-2xl font-bold text-white">模拟秒合约交易管理</h1>
        <p className="text-gray-400 mt-1">查看和管理模拟账户的秒合约交易记录</p>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-gray-400">模拟交易数</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-gray-400">模拟盈亏</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
          <CardTitle className="text-white">模拟秒合约交易记录</CardTitle>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex items-center justify-center py-8">
              <div className="text-gray-400">暂无模拟秒合约交易记录</div>
            </div>
          ) : (
