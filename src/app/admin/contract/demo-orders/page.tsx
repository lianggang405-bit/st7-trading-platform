...
export default function DemoContractOrdersPage() {
  const [orders, setOrders] = useState<DemoContractOrder[]>([]);
  ...
  const [viewOrder, setViewOrder] = useState<DemoContractOrder | null>(null);
  const [editOrder, setEditOrder] = useState<DemoContractOrder | null>(null);
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/contract/demo-orders?page=${currentPage}&limit=${pageSize}&sort=${sortField}&order=${sortOrder}&search=${searchQuery}`
      );
  ...
  const handleDelete = async (orderId: number) => {
    if (!confirm('确定要删除此模拟订单吗？')) return;

    try {
      const response = await fetch(`/api/admin/contract/demo-orders/${orderId}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      if (data.success) {
      ...
      console.error('Failed to delete demo order:', error);
      toast.error('删除失败');
    }
  };

  const handleView = (order: DemoContractOrder) => {
    setViewOrder(order);
    setViewDialogOpen(true);
  };

  const handleEdit = (order: DemoContractOrder) => {
    setEditOrder(order);
    setEditForm({
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      const response = await fetch('/api/admin/contract/demo-orders', {
        method: 'POST',
        headers: {
  ...
  const handleEditSubmit = async () => {
    ...
    setEditLoading(true);
    try {
      const response = await fetch(`/api/admin/contract/demo-orders/${editOrder.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('Failed to update demo order:', error);
      toast.error('更新失败');
    } finally {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { color: string; text: string }> = {
      ...
      挂单中: { color: 'bg-yellow-500/10 text-yellow-400', text: '挂单中' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400">
        <span>资源</span> / <span className="text-white">模拟合约订单</span>
      </div>

      {/* 页面标题和操作栏 */}
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">模拟合约订单</h1>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>创建模拟合约订单</DialogTitle>
            <DialogDescription className="text-gray-400">
              创建新的模拟合约订单
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label htmlFor="create-account">账号 *</Label>
              <Input
                id="create-account"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="create-symbol">品种</Label>
                <select
                  id="create-symbol"
              ...
              <div>
                <Label htmlFor="create-tradeType">交易类型</Label>
                <select
                  id="create-tradeType"
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <Label htmlFor="create-originalPrice">原始价 *</Label>
                <Input
              ...
              <div>
                <Label htmlFor="create-openPrice">开仓价格</Label>
                <Input
                  id="create-openPrice"
                  type="number"
              ...
              <div>
                <Label htmlFor="create-currentPrice">当前价格</Label>
                <Input
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="create-takeProfit">止盈价</Label>
                <Input
              ...
              <div>
                <Label htmlFor="create-stopLoss">止损价</Label>
                <Input
                  id="create-stopLoss"
                  type="number"
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <Label htmlFor="create-lots">手数 *</Label>
                <Input
                  id="create-lots"
                  type="number"
                  step="0.01"
                  value={createForm.lots}
                  onChange={(e) => setCreateForm({ ...createForm, lots: parseFloat(e.target.value) || 0 })}
                  className="bg-slate-700 border-slate-600 text-white mt-1"
                />
              </div>
              <div>
                <Label htmlFor="create-leverage">倍数</Label>
                <Input
                  id="create-leverage"
                  type="number"
                  value={createForm.leverage}
                  onChange={(e) => setCreateForm({ ...createForm, leverage: parseInt(e.target.value) || 10 })}
                  className="bg-slate-700 border-slate-600 text-white mt-1"
                />
              </div>
              <div>
                <Label htmlFor="create-fee">手续费</Label>
                <Input
                  id="create-fee"
                  type="number"
                  step="0.0001"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="create-initialMargin">初始保证金</Label>
                <Input
              ...
              <div>
                <Label htmlFor="create-availableMargin">可用保证金</Label>
                <Input
