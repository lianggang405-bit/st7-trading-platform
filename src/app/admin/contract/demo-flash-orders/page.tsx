...
export default function DemoFlashOrdersPage() {
  const [orders, setOrders] = useState<DemoFlashOrder[]>([]);
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/contract/demo-flash-orders?page=${currentPage}&limit=${pageSize}&sort=${sortField}&order=${sortOrder}&search=${searchQuery}`
      );
      ...
      console.error('Failed to fetch demo flash orders:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (orderId: number) => {
    if (!confirm('确定要删除此模拟秒合约订单吗？')) return;

    try {
      const response = await fetch(`/api/admin/contract/demo-flash-orders/${orderId}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      if (data.success) {
      ...
      console.error('Failed to delete demo flash order:', error);
      toast.error('删除失败');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400">
        <span>资源</span> / <span className="text-white">模拟秒合约交易</span>
      </div>

      {/* 页面标题和操作栏 */}
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">模拟秒合约交易</h1>
