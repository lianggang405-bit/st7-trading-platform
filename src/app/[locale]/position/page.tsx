...
export default function PositionPage() {
  ...
  const handleClosePosition = (id: string) => {
    ...
    const direction = position.side === 'buy' ? t('buy') : t('sell');
    ...
    setConfirmDialog({
      ...
      description: (
        <div className="space-y-2">
          ...
          <div className="flex justify-between">
            <span className="text-gray-500">{t('openPrice')}:</span>
      ...
      onConfirm: async () => {
        ...
        if (!result.success) {
          console.error(t('error.closeFailed'), result.error);
          alert(`${t('error.closeFailed')}: ${result.error}`);
        }
      },
    });
  };
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
              ...
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 mb-6 shadow-sm border border-gray-200">
                ...
                <div className="text-center mb-6">
                  <p className="text-gray-500 text-sm mb-1">{t('profit')}</p>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500 text-sm">{t('currentMargin')}</span>
                    <span className="text-gray-900 font-semibold">{usedMargin.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500 text-sm">{t('availableMargin')}</span>
                    <span className="text-gray-900 font-semibold">{freeMargin.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500 text-sm">{t('riskRatio')}</span>
              ...
              {danger && (
                <div className="mb-4 bg-red-50 border border-red-200 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-red-600 text-sm">
                    ...
                    <span className="font-medium">{t('danger')}</span>
                  </div>
                </div>
              )}
              ...
              {positions.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
                  ...
                  <p className="text-gray-400 text-lg font-medium">{t('noPositions')}</p>
                </div>
              ) : (
                <div className="space-y-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {position.side === 'buy' ? t('buy') : t('sell')}
                              </span>
                          ...
                          <div className="grid grid-cols-3 gap-3 mb-3">
                            ...
                            <div>
                              <p className="text-xs text-gray-400 mb-1">{t('openPrice')}</p>
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-gray-400">{t('profit')}</span>
