...
export async function POST(
  ...
  try {
    ...
    if (error && error.message && error.message.includes('schema cache')) {
      console.log('[DepositRequests Approve] Schema cache error detected, trying to refresh...');
      
      // 刷新 schema cache：通过执行一个简单的查询来刷新
      try {
        await supabase.from('deposit_requests').select('id').limit(1);
        console.log('[DepositRequests Approve] Schema cache refreshed, retrying update...');
        ...
        console.error('[DepositRequests Approve] Retry also failed:', retryError);
        // 保持原始错误
      }
    }
