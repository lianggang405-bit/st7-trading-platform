...
- **最新需求**: 修复K线图价格与交易对价格不一致的问题
  - 比特币交易对价格是10万，但K线只有5万多
  - 原因：K线使用硬编码的基础价格，而不是实时价格
  - 解决方案：K线初始化时等待实时价格加载完成，使用实时价格作为初始价格
- **之前需求**: 添加K线图图表标签国际化支持
...
- 问题: K线图价格与交易对价格不一致
  - **问题**: K线使用硬编码的基础价格（BTCUSD = 98500），而交易对价格是实时价格（100000+）
  - **根因**: 
    - K线初始化时使用 `getBasePrice(symbol)` 返回的硬编码价格
    - 实时价格加载后，K线已经在错误的价格水平上生成历史数据
    - 导致K线价格（5万多）与交易对价格（10万）相差很大
  - **解决方案**: 
    - K线初始化时等待实时价格加载完成（`currentSymbolPrice > 0`）
    - 使用实时价格作为K线的初始价格，而不是硬编码的基础价格
    - 移除对 `getBasePrice` 函数的依赖
    - 确保K线价格与交易对价格完全一致
