...
- 问题: 用户要求恢复 K 线图到修改之前的版本
  ...
  - **解决方案**: 
    ...
    - 恢复原始的波动幅度（100/50/20）
- 问题: TypeScript 类型错误
  - **问题**: `addCandlestickSeries` 方法类型定义问题
  - **解决方案**: 使用类型断言 `(chart as any).addCandlestickSeries()`
- 问题: 时间类型错误
  - **问题**: `data.time` 可能是 `BusinessDay` 类型
  - **解决方案**: 使用 `@ts-ignore` 绕过类型检查
